...
export default class InformasiTeknologi extends Component {

    static navigationOptions = {
        title: 'InformasiTeknologi',
        headerStyle: {
            elevation: 0
        },
    };
    ...
    constructor(props) {
        ...
        AsyncStorage.getItem('@Student_NIS').then(value => {
            firebase.database().ref("/InformasiTeknologi").child(value).on('value', (childSnapshot) => {
                const tahfizhrecord = [];
            ...
            firebase.database().ref("/AssignedTeacher").on('value', (childSnapshot) => {
                let teacher_list = [];
                childSnapshot.forEach((doc) => {
                    if (doc.toJSON().subject == 'InformasiTeknologi') {
                        teacher_list.push({
                            id: doc.key,
            ...
            <View style={styles.maincontainer}>
                <View
                    ...
                    <Text style={{
                        ...
                        fontSize: 16,
                        color: '#777777'
                    }}>Pendamping InformasiTeknologi</Text>
                    <Br height={8} />
                    <FlatList
